#!/bin/bash
#SBATCH -Jeval_orig_density
#SBATCH --output=logs/eval_orig_density_rules_%A_%a.out
#SBATCH --error=logs/eval_orig_density_rules_%A_%a.err
...
# with density_sampling rules (oracle subtask labels).
# Isolates whether density rules help/hurt on a well-trained VLA.
# Creates a per-job symlink tree to override history_config.txt without races.
# Array job: 8 tasks, each handles 2 of the 16 tasks.
#
# Usage:
#   sbatch slurm_eval_original_density_rules_sim.sh             # all 8 array jobs
#   sbatch --array=0 slurm_eval_original_density_rules_sim.sh   # just first pair

# ---------- Configuration ----------
METHOD="original_density_rules"
...
# ---- Create per-job wrapper directory with density history_config ----
# Override history_config.txt to use density_sampling on the uniform-trained VLA.
JOB_TMP_DIR="/tmp/orig_density_rules_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "$JOB_TMP_DIR"
printf "perceptual-density-modul.yaml" > "$JOB_TMP_DIR/history_config.txt"
...
TASKS:            $TASKS
METHOD:           $METHOD
BASE_MODEL:       $BASE_MODEL_TYPE (with density_sampling override)
CKPT_ID:          $CKPT_ID
SEED:             $SEED
PORT:             $PORT
EOF
...
echo "Tasks: $TASKS"
echo "VLA checkpoint: $BASE_MODEL_TYPE/$CKPT_ID (original released, density rules override)"
echo "Using tmp dir: $JOB_TMP_DIR"
nvidia-smi

# ---- Launch policy server on GPU_SERVER ----
echo "Starting policy server on GPU $GPU_SERVER (original VLA, density rules) ..."
