#!/bin/bash
#SBATCH -Jeval_uniform_sim
#SBATCH --output=logs/eval_uniform_sim_%A_%a.out
#SBATCH --error=logs/eval_uniform_sim_%A_%a.err
...
# Evaluate retrained uniform-sampling VLA baseline in simulation.
# No selector — uses default frame_sampling (uniform) from the VLA config.
# Array job: 8 tasks, each handles 2 of the 16 tasks.
#
# Usage:
#   sbatch slurm_eval_uniform_sim.sh             # all 8 array jobs, default ckpt
#   sbatch --array=0 slurm_eval_uniform_sim.sh   # just first pair
#   sbatch slurm_eval_uniform_sim.sh 15000        # custom ckpt id

CKPT_ID="${1:-15000}"

# ---------- Configuration ----------
METHOD="uniform_retrain"
BASE_MODEL_TYPE="perceptual-framesamp-modul-retrain"
