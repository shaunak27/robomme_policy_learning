#!/bin/bash
#SBATCH -Jeval_density_rules
#SBATCH --output=logs/eval_density_rules_%A_%a.out
#SBATCH --error=logs/eval_density_rules_%A_%a.err
...
# Evaluate density VLA with oracle rule-based density sampling in simulation.
# No learned selector — uses density_sampling rules with oracle subtask labels
# from the simulator (simple_subgoal_oracle).
# Array job: 8 tasks, each handles 2 of the 16 tasks.
#
# Usage:
#   sbatch slurm_eval_density_rules_sim.sh              # all 8 array jobs, default ckpt
#   sbatch --array=0 slurm_eval_density_rules_sim.sh    # just first pair
#   sbatch slurm_eval_density_rules_sim.sh 17500         # custom ckpt id

CKPT_ID="${1:-19999}"

# ---------- Configuration ----------
METHOD="density_rules"
...
# No selector — density VLA uses its built-in density_sampling rules
# with oracle subtask labels provided by the eval loop
echo "Starting policy server on GPU $GPU_SERVER (density rules, no learned selector) ..."
