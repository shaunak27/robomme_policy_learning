#!/bin/bash
#SBATCH -Jeval_density
#SBATCH --output=logs/eval_density_%j.out
#SBATCH --error=logs/eval_density_%j.err
...
# ---------- Configuration ----------
TASK="${1:-all}"                   # pass task name as first arg, default all
METHOD="density"
...
# Create a wrapper checkpoint dir that reuses base model weights
# but overrides history_config to density_sampling
DENSITY_CKPT_DIR="runs/ckpts/$CONFIG_TYPE/density-eval-wrapper"
mkdir -p "$DENSITY_CKPT_DIR/$CKPT_ID"

# Symlink the actual model params
ln -sfn "$(realpath runs/ckpts/$CONFIG_TYPE/$BASE_MODEL_TYPE/$CKPT_ID/params)" \
    "$DENSITY_CKPT_DIR/$CKPT_ID/params"

# Symlink assets if they exist
if [ -d "runs/ckpts/$CONFIG_TYPE/$BASE_MODEL_TYPE/$CKPT_ID/assets" ]; then
    ln -sfn "$(realpath runs/ckpts/$CONFIG_TYPE/$BASE_MODEL_TYPE/$CKPT_ID/assets)" \
        "$DENSITY_CKPT_DIR/$CKPT_ID/assets"
fi

# Override history config to density sampling
echo -n "perceptual-density-modul.yaml" > "$DENSITY_CKPT_DIR/history_config.txt"
...
CUDA_VISIBLE_DEVICES=$GPU_SERVER /coc/testnvme/shalbe3/miniconda/envs/robomme/bin/uv run scripts/serve_policy.py \
    ...
    --port=$PORT \
    policy:checkpoint \
    --policy.dir=$DENSITY_CKPT_DIR/$CKPT_ID \
...
if [ "$TASK" != "all" ]; then
    EVAL_ARGS+=(--args.only-tasks=$TASK)
fi
