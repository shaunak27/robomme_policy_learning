#!/bin/bash
#SBATCH -Jeval_sel_patternlock
#SBATCH --output=logs/eval_selector_patternlock_%j.out
#SBATCH --error=logs/eval_selector_patternlock_%j.err
...
CKPT_ID=79999
SEED=42
SELECTOR_CKPT="runs/ckpts/rl_selector/final"
GPU_SERVER=0
GPU_CLIENT=1
# -----------------------------------
...
CUDA_VISIBLE_DEVICES=$GPU_SERVER /coc/testnvme/shalbe3/miniconda/envs/robomme/bin/uv run scripts/serve_policy.py \
    --seed=$SEED \
    --port=$PORT \
    --selector-checkpoint=$SELECTOR_CKPT \
    policy:checkpoint \
