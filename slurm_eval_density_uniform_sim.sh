#!/bin/bash
#SBATCH -Jeval_density_unif
#SBATCH --output=logs/eval_density_uniform_%A_%a.out
#SBATCH --error=logs/eval_density_uniform_%A_%a.err
...
# Evaluate density VLA with UNIFORM frame sampling (not density rules).
# Isolates whether the density VLA weights are the problem vs the sampling strategy.
# Creates a per-job symlink tree so we can override history_config.txt without races.
# Array job: 8 tasks, each handles 2 of the 16 tasks.
#
# Usage:
#   sbatch slurm_eval_density_uniform_sim.sh              # all 8 array jobs, default ckpt
#   sbatch slurm_eval_density_uniform_sim.sh 17500         # custom ckpt id

CKPT_ID="${1:-19999}"

# ---------- Configuration ----------
METHOD="density_vla_uniform"
...
ORIG_CKPT_DIR="runs/ckpts/$CONFIG_TYPE/$BASE_MODEL_TYPE"

# ---- Create per-job wrapper directory with uniform history_config ----
# This avoids races between array jobs writing to the same file.
# Structure: tmp dir has history_config.txt (uniform) + symlink to real ckpt
JOB_TMP_DIR="/tmp/density_uniform_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "$JOB_TMP_DIR"
printf "perceptual-framesamp-modul.yaml" > "$JOB_TMP_DIR/history_config.txt"
ln -sfn "$(realpath $ORIG_CKPT_DIR/$CKPT_ID)" "$JOB_TMP_DIR/$CKPT_ID"

cleanup_tmp() {
    rm -rf "$JOB_TMP_DIR"
    echo "Cleaned up $JOB_TMP_DIR"
}
trap cleanup_tmp EXIT

VLA_CKPT_DIR="$JOB_TMP_DIR"
...
TASKS:            $TASKS
METHOD:           $METHOD
BASE_MODEL:       $BASE_MODEL_TYPE (with uniform frame_sampling override)
CKPT_ID:          $CKPT_ID
SEED:             $SEED
PORT:             $PORT
EOF
...
echo "Tasks: $TASKS"
echo "VLA checkpoint: $BASE_MODEL_TYPE/$CKPT_ID (uniform sampling override)"
echo "Using tmp dir: $JOB_TMP_DIR"
nvidia-smi

# ---- Launch policy server on GPU_SERVER ----
echo "Starting policy server on GPU $GPU_SERVER (density VLA, uniform sampling) ..."
